...
object TestData {
    /**
     * Creates a chapter node (zc type)
     * Format: ["zc", chapterTitle, chapterNumber]
     */
    fun createChapterNode(id: String, chapterTitle: String, chapterNumber: String): Node {
        return Node(
            id = id,
            data = JSONArray(listOf("zc", chapterTitle, chapterNumber)),
        )
    }

    /**
     * Creates a paragraph node with text and optional verse markers
     * Format: ["p", spansArray]
     * Spans can be strings (text) or arrays like ["v", verseNumber] for verse markers
     */
    ...
    /**
     * Creates a heading node (s1 type)
     * Format: ["s1", spansArray]
     */
    fun createHeadingNode(
        id: String,
        text: String,
    ): Node {
        val spansArray = JSONArray().apply {
            put(text)
        }
        return Node(
            id = id,
    ...
    /**
     * Creates a verse marker span
     * Format: ["v", verseNumber]
     */
