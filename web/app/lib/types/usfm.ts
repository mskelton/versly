// Introductions
// https://ubsicap.github.io/usfm/introductions/index.html
export type Introduction = [type: 'iex', children: Span[]]

// Titles, Headings, and Labels
// https://ubsicap.github.io/usfm/titles_headings/index.html
export type MajorSection = [type: 'ms', children: Span[]]
export type Section = [type: 's1' | 's2' | 's3', children: Span[]]
export type DescriptiveTitle = [type: 'd', children: Span[]]
export type Speaker = [type: 'sp', children: Span[]]

// Chapters and Verses
// https://ubsicap.github.io/usfm/chapters_verses/index.html
export type ChapterNumber = [type: 'c', content: string]
export type VerseNumber = [type: 'v', content: string]

// Paragraphs
// https://ubsicap.github.io/usfm/paragraphs/index.html
export type Paragraph = [type: 'p', children: Span[]]
export type ContinuationParagraph = [type: 'm', children: Span[]]
export type ParagraphRightAligned = [type: 'pr', children: Span[]]
export type Closure = [type: 'cls', children: Span[]]
export type EmbeddedTextOpening = [type: 'pmo', children: Span[]]
export type EmbeddedText = [type: 'pm', children: Span[]]
export type EmbeddedTextClosing = [type: 'pmc', children: Span[]]
export type EmbeddedTextRefrain = [type: 'pmr', children: Span[]]
export type IndentedParagraph = [type: 'pi1' | 'pi2' | 'pi3', children: Span[]]
export type IndentedFlushLeftParagraph = [type: 'mi', children: Span[]]
export type NoBreak = [type: 'nb', children: Span[]]
export type ParagraphCentered = [type: 'pc', children: Span[]]
export type BlankLine = [type: 'b']

// Poetry
// https://ubsicap.github.io/usfm/poetry/index.html
export type PoeticLine = [type: 'q1' | 'q2' | 'q3' | 'q4', children: Span[]]
export type PoeticLineRightAligned = [type: 'qr', children: Span[]]
export type PoeticLineCentered = [type: 'qc', children: Span[]]
export type Selah = [type: 'qs', content: string]
export type AcrosticHeading = [type: 'qa', children: Span[]]
export type AcrosticCharacter = [type: 'qac', content: string]
export type EmbeddedTextPoeticLine = [type: 'qm1' | 'qm2', children: Span[]]

// Lists
// https://ubsicap.github.io/usfm/lists/index.html
export type ListItem = [type: 'li1' | 'li2' | 'li3' | 'li4', children: Span[]]
export type EmbeddedListItem = [type: 'lim', children: Span[]]
export type ListEntryTotal = [type: 'litl', content: string]

// Tables
// https://ubsicap.github.io/usfm/tables/index.html
export type Table = [type: 'table', rows: (TableHeading[] | TableCell[])[]]
export type TableHeading = [type: 'th', children: Span[]]
export type TableCell = [type: 'td', children: Span[]]

// Special Text
// https://ubsicap.github.io/usfm/characters/index.html#special-text
export type TranslatorAddition = [type: 'add', content: string]
export type BookTitle = [type: 'bk', content: string]
export type NameOfGod = [type: 'nd', content: string]
export type QuotedText = [type: 'qt', content: string]
export type Signature = [type: 'sig', content: string]
export type SecondaryLanguageSource = [type: 'sls', content: string]
export type Transliteration = [type: 'tl', content: string]
export type WordsOfJesus = [type: 'wj', content: string]

// Character Styling
// https://ubsicap.github.io/usfm/characters/index.html#character-styling
export type EmphasisText = [type: 'em', content: string]
export type Bold = [type: 'bd', content: string]
export type Italic = [type: 'it', content: string]
export type NormalText = [type: 'no', content: string]
export type SmallCap = [type: 'sc', content: string]
export type Superscript = [type: 'sup', content: string]

export type Node =
	| Introduction
	| MajorSection
	| Section
	| DescriptiveTitle
	| Speaker
	| ChapterNumber
	| Paragraph
	| ContinuationParagraph
	| ParagraphRightAligned
	| Closure
	| EmbeddedTextOpening
	| EmbeddedText
	| EmbeddedTextClosing
	| EmbeddedTextRefrain
	| IndentedParagraph
	| IndentedFlushLeftParagraph
	| NoBreak
	| ParagraphCentered
	| BlankLine
	| PoeticLine
	| PoeticLineRightAligned
	| PoeticLineCentered
	| AcrosticHeading
	| EmbeddedTextPoeticLine
	| ListItem
	| EmbeddedListItem
	| Table

export type Span =
	| string
	| VerseNumber
	| ListEntryTotal
	| TranslatorAddition
	| BookTitle
	| Selah
	| AcrosticCharacter
	| NameOfGod
	| QuotedText
	| Signature
	| SecondaryLanguageSource
	| Transliteration
	| WordsOfJesus
	| EmphasisText
	| Bold
	| Italic
	| NormalText
	| SmallCap
	| Superscript

export type NodeType = Node[0] | Span[0] | TableHeading[0] | TableCell[0]
