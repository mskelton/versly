...
export function Progress({ passages }: { passages: Passage[] }) {
  return (
    <div className="flex gap-2 w-full py-2 px-6 sticky top-0 mt-6 bg-white dark:text-gray-50 dark:bg-neutral-950 z-10">
...
function ProgresPill({ passage }: { passage: Passage }) {
  const { chapter } = parsePassageId(passage.id)!

  return (
    <div className="rounded-full bg-neutral-200 dark:bg-neutral-800 px-4 text-xs py-1 w-full text-center font-semibold relative">
      <span
