...
import { Passage, PassageId, BookMetadata, Translation, Node } from '../models/types';
...
class BibleDatabase {
  ...
  async downloadTranslation(translationId: string, data: string[]): Promise<void> {
        ...
        switch (type) {
          ...
          case 'r':
            await tx.executeSql(
              ...
              [parsed[1], parsed[2], parsed[3], parsed[4], parsed[5], translationId]
            );
            break;
        }
      }
  ...
  async getPassage(book: string, chapter: string, translation: string): Promise<Passage> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async getNextChapter(book: string, chapter: string, translation: string): Promise<PassageId | null> {
    if (!this.db) throw new Error('Database not initialized');
  ...
  async getPreviousChapter(book: string, chapter: string, translation: string): Promise<PassageId | null> {
    if (!this.db) throw new Error('Database not initialized');
